 ...
 */
const defaultIsAssetManifestIcon = (fileName) => !!fileName.match(/manifest\/icon_\d+-\w*\.(png|jpeg|jpg)$/);
/**
...
const defaultGetIconSize = (fileName) => {
    ...
    if (!dimension || Number.isNaN(dimension)) {
        throw new Error(`Invalid icon dimension found ${JSON.stringify(dimension)} in filename ${JSON.stringify(fileName)}`);
    }
...
const defaultGetIconType = (fileName) => {
    ...
    /* istanbul ignore if */
    if (!extension) {
        throw new Error(`Invalid icon extension found ${JSON.stringify(extension)} in filename ${JSON.stringify(fileName)}`);
    }
...
module.exports = class WebAppManifestPlugin {
     ...
     */
    constructor({ content, destination, isAssetManifestIcon = defaultIsAssetManifestIcon, getIconSize = defaultGetIconSize, getIconType = defaultGetIconType, }) {
